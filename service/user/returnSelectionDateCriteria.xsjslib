...
var returnSelectionDateCriteriaService = registrationObjectLib.registrationObjectService;
...
returnSelectionDateCriteriaService.setModel(new model.RegistrationObject());

 
returnSelectionDateCriteriaService.create = function(params) {
    ...
    try {
        ...
        regiProc = new proc.Procedure(SCHEMA, 'sap.tm.trp.db.pickupreturn::p_return_register_selection_date_criteria', {
            connection: conn
        });
...
returnSelectionDateCriteriaService.update = function(params) {
    ...
    try {
        conn = $.db.getConnection($.db.isolation.SERIALIZABLE);
        conn.setAutoCommit(false);
        updateProc = new proc.Procedure('SAP_TM_TRP', 'sap.tm.trp.db.pickupreturn::p_return_register_selection_date_criteria_update', {
            connection: conn
        });
...
returnSelectionDateCriteriaService.destroy = function(params) {
    ...
    try {
        ...
        conn = $.db.getConnection($.db.isolation.SERIALIZABLE);
        conn.setAutoCommit(false);
        deleteProc = new proc.Procedure('SAP_TM_TRP', 'sap.tm.trp.db.pickupreturn::p_return_register_selection_date_criteria_delete', {
            connection: conn
        });
        deleteProc(entryID);
...
//returnSelectionDateCriteriaService.setFilters([{
//    filter: function(params) {
//        var privilege = "sap.tm.trp.service::RegisterReturnSelectionDateCriteria";
...
//}, {
//    filter: function(params) {
//        var privilege = "sap.tm.trp.service::EditReturnSelectionDateCriteria";
//        if (!$.session.hasAppPrivilege(privilege)) {
...
//         if (isValid === 0) {
//             logger.error("OBJECT_REGISTRATION_RETURN_FAILED",
//                 params.obj.SCHEMA,
...
try {
    returnSelectionDateCriteriaService.handle();
} finally {
    logger.close();
}