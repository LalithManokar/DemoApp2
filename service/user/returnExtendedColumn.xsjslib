...
var returnExtColModelService = registrationObjectLib.registrationObjectService;
...
returnExtColModelService.setModel(new model.RegistrationObject());

 
returnExtColModelService.create = function(params) {
    ...
    try {
        ...
        regiProc = new proc.Procedure(SCHEMA, 'sap.tm.trp.db.pickupreturn::p_register_return_extended_col_create', {
            connection: conn
        });
...
returnExtColModelService.update = function(params) {
    ...
    try {
        conn = $.db.getConnection($.db.isolation.SERIALIZABLE);
        conn.setAutoCommit(false);
        updateProc = new proc.Procedure('SAP_TM_TRP', 'sap.tm.trp.db.pickupreturn::p_register_return_extended_col_update', {
            connection: conn
        });
...
returnExtColModelService.regenerate = function(params) {
    ...
    try {
        conn = $.db.getConnection($.db.isolation.SERIALIZABLE);
        conn.setAutoCommit(false);
        regenerateProc = new proc.Procedure('SAP_TM_TRP', 'sap.tm.trp.db.pickupreturn::p_register_retrun_extended_col_regenerate', {
            connection: conn
        });
...
returnExtColModelService.setFilters([{
    filter: function(params) {
        var privilege = "sap.tm.trp.service::RegisterReturnExtendedColumn";
    ...
    filter: function(params) {
        var privilege = "sap.tm.trp.service::EditReturnExtendedColumn";
        if (!$.session.hasAppPrivilege(privilege)) {
...
try {
    returnExtColModelService.handle();
} finally {
    logger.close();
}