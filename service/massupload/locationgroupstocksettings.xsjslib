...
var parseCSVFiles = function(content) {
    try {
        ...
        anti.scan(content, "location_group_stock_settings.csv");
    } catch (e) {
        logger.error("LOCATION_GROUP_STOCK_SETTINGS_FILE_ANTIVIRUS_FAILED",
                'Location Group Stock Settings', e);
    ...
    try {
        ...
        logger.error("LOCATION_GROUP_STOCK_SETTINGS_FILE_PARSE_FAILED", e);
...
var formatCSVFiles = function(data) {
    ...
    for (i = 0; i < data.length; i++) {
        content = data[i];
        if (content.length !== 5) {
            logger.error("LOCATION_GROUP_STOCK_SETTINGS_FILE_VALIDATION_COLUMN_COUNT_FAILED",
                    'Location Group Stock Settings', i, content.length, 5);
        ...
        {
        	csvErrorList.push(csvMessage('LOCATION_GROUP_STOCK_NAME_GT_20',
                     obj.ROW_INDEX, 1));  
        }else {
        ...
        {
        	csvErrorList.push(csvMessage('LOCATION_GROUP_STOCK_RESOURCE_TYPE_GT_20',
                     obj.ROW_INDEX, 2));  
        }else {
...
var checkLocationGroupStockSettingsName = function() {
    return "p_location_grp_stock_settings_validate";
};

var saveLocationGroupStockSettingsName = function() {
    return "p_location_grp_stock_settings_save";
};

var uploadLocationGroupStockSettingsName = function() {
    return "p_location_grp_stock_settings_upload";
};

var cleanLocationGroupStockSettingsName = function() {
    return "p_location_grp_stock_settings_temporary_data_clean";
};
...
var upload = function(csv, resourceCategory, connectionId) {
    ...
    var checProcName = checkLocationGroupStockSettingsName();

    // return different check procedure name according to the type
    var uploadProcName = uploadLocationGroupStockSettingsName();
...
var save = function(resourceCategory, connectionId) {    
    ...
    try {
        procName = saveLocationGroupStockSettingsName();
        ...
        logger.error("LOCATION_GROUP_STOCK_SETTINGS_SAVE_FAILED",
                connectionId, e);
